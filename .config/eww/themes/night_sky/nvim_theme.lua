...
-- UI options the theme expects (line numbers, cursorline, etc.)
...
-- Lualine setup table (mode pill per vim mode, plain separators)
...
-- Bufferline setup table (transparent tabs)
function M.bufferline_opts()
  local p = M.palette
  return {
    options = {
    ...
    highlights = {
      ...
      buffer_selected = { bg = p.surface, fg = p.peach, bold = true },
      buffer_visible = { bg = "NONE", fg = p.muted },
      separator = { bg = "NONE", fg = p.muted },
      separator_selected = { bg = p.surface, fg = p.muted },
      indicator_selected = { bg = p.surface, fg = p.violet },
...
-- Notify setup table (solid bg: Normal is transparent for kitty blur)
function M.notify_opts()
  return { background_colour = M.palette.float }
end

-- indent-blankline setup table (uses the IblIndent / IblScope groups above)
function M.ibl_opts()
  return {
